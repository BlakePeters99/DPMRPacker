   ...
   public Result packItems(Item[] items, int maxWeight, boolean verbose) {
      ...
      for (int item = 1; item <= items.length; item++) {
         for (int weight = 0; weight <= maxWeight; weight++) {
            ...
            // if item can fit in slot and the current slot is less than
            // table[previous row] [current column - item's weight ] + item's value
            if (weight >= items[item - 1].getWeight() &&
             table[item][weight] < table[item - 1][weight - items[item - 1]
              .getWeight()] + items[item - 1].getValue()) {
               // then make slot the new greater value
               table[item][weight] = table[item - 1][weight -
                items[item - 1].getWeight()] + items[item - 1].getValue();
            }
      ...
      while (numItems > 0) {
         if (table[numItems][breadcrumbWeight] !=
          table[numItems-1][breadcrumbWeight]) {
            currentWeight = items[numItems-1].getWeight();
            solution.add((Item) new PackerTester.MyItem(
             items[numItems-1].getValue(), currentWeight));
            newWeight += currentWeight;
            breadcrumbWeight -= currentWeight;
         }
         numItems--;
      }
      Stats stats = new Stats(table[items.length][maxWeight],
       newWeight, items.length, 0);
